package com.sbn.rest.exception.handler;
...
public class RestExceptionHandler {
	...
	public ResponseEntity<ErrorMessage> MethodArgumentNotValidException(MethodArgumentNotValidException  ex, WebRequest request) {
		...
		ErrorMessage errorObj = new ErrorMessage(new Date(), "Arguments non valide : " + ex.getBindingResult().getAllErrors().stream().map(elm ->  elm.getDefaultMessage()).collect (Collectors.joining (",")), request.getDescription(false));
	...
	public ResponseEntity<ErrorMessage> handleAmpsRequestNotFoundException(AmpsRequestNotFoundException ex, WebRequest request) {
